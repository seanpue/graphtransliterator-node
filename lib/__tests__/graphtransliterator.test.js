...
var GraphTransliterator = graphtransliterator.GraphTransliterator;
...
describe("graphtransliterator", () => {
  ...
  it("can matchAt", () => {
    var tokens = gt.tokenize("ab");
    var justTokens = tokens.map(el => el.token);
    console.log(gt.matchAt(1, justTokens));
    console.log(gt.matchAt(2, justTokens));
    assert(gt.matchAt(1, justTokens) >= 0);
  });
  it("can tranliterate", () => {
